...
public final class FibonacciHeap<V> {
    ...
    public static final class Entry<V> {
        ...
        private Entry<V> mNext;   // Next and previous elements in the list
        private Entry<V> mPrev;

        private Entry<V> mParent; // Parent in the tree, if any.
    ...
    public Entry<V> dequeueMin() {
         ...
         */
        List<Entry<V>> treeTable = new ArrayList<>();
         ...
         */
        List<Entry<V>> toVisit = new ArrayList<>();
        ...
        for (Entry<V> curr: toVisit) {
            ...
            while (true) {
                ...
                Entry<V> other = treeTable.get(curr.mDegree);
                 ...
                 */
                Entry<V> min = (other.mPriority < curr.mPriority)? other : curr;
                Entry<V> max = (other.mPriority < curr.mPriority)? curr  : other;
    ...
    public void delete(Entry<V> entry) {
         ...
         */
        decreaseKeyUnchecked(entry, Float.NEGATIVE_INFINITY);
     ...
     * it's a valid float and throws an IllegalArgumentException otherwise.
     *
    ...
    private static <V> Entry<V> mergeLists(Entry<V> one, Entry<V> two) {
        ...
        if (one == null && two == null) { // Both null, resulting list is null.
             ...
             *
             */
            Entry<V> oneNext = one.mNext; // Cache this since we're about to overwrite it.
            one.mNext = two.mNext;
            one.mNext.mPrev = one;
            two.mNext = oneNext;
            two.mNext.mPrev = two;
     ...
     */
    private void decreaseKeyUnchecked(Entry<V> entry, float priority) {
